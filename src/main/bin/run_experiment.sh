...
input_checks() {
	if [ "$1" == "-h" ]; then
			echo "Usage: $(basename "${0}") {WORK_FLOW = (SQL, RDD, BUCKET, HIVE, INDEXED)} {BASE_PATH} {EXPERIMENT NUMBER - can be e1, e2 or e3} {NUMBER OF PARTITIONS}"
			exit 0
	fi
...
main() {
  ...
  "INDEXED")
    ...
    spark-submit \
    --class edu.asu.hyperspacetest.Main \
    --master spark://"${SPARK_MASTER}" \
    --conf spark.default.parallelism="${SPARK_DEFAULT_PAR}" \
    --deploy-mode client \
    "${APP_HOME}"/lib/Spark-Partitioning-0.1-SNAPSHOT.jar \
