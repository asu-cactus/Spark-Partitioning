...
object MultiplyBlockedMatrix {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (!useBreeze) {
        ...
        val left_blocks = sc.objectFile[((Int, Int), breeze.linalg.DenseMatrix[Double])](inputLeftFilePath).cache()
        val right_blocks = sc.objectFile[((Int, Int), breeze.linalg.DenseMatrix[Double])](inputRightFilePath).cache()
        val lblocks = left_blocks.map({case block => (block._1._2, (block._1._1, block))})
        val rblocks = right_blocks.map({case block => (block._1._1, (block._1._2, block))})
        val P : org.apache.spark.rdd.RDD[((Int, Int), breeze.linalg.DenseMatrix[Double])] = lblocks.join(rblocks).map({ case (_, ((r, lb), (c, rb))) => 
          val B: breeze.linalg.DenseMatrix[Double] = rb._2 match {
              case dense: breeze.linalg.DenseMatrix[Double] => lb._2 * dense
              case _ => throw new SparkException(s"Unrecognized matrix type ${rb._2.getClass}.")
          }
          ((r, c), B)}).reduceByKey((a, b) => a + b)
